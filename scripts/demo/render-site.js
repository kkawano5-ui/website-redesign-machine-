...
function voiceCards(v) {
  return v.voices
    .map(
      (vo) => `
        <article class="voice">
          <p>「${esc(vo.text)}」</p>
          <span>— ${esc(vo.who)} <i>（サンプル）</i></span>
        </article>`
    )
    .join('');
}

// company: { name, area, vertical(key), reviewCount, website, placeId }
export function renderSite(company) {
  const v = VERTICALS[company.vertical];
  if (!v) throw new Error(`未知の業種キー: ${company.vertical}`);

  const c = {
    name: company.name || '貴社名',
    area: company.area || 'お住まいの地域',
  };
  const reviewChip =
    Number(company.reviewCount) > 0
      ? `<span class="chip">★ Googleクチコミ ${esc(company.reviewCount)}件</span>`
      : '';
  const title = `${c.name}｜${v.label}（${c.area}）`;
  const desc = `${c.area}の${v.label}。${c.name}のサービス・特長・アクセスのご案内。`;
...
<!-- 営業提案用サンプル。写真・実績・お客様の声・料金・連絡先は差し替え前提（掲載情報は捏造しない）。業種別NG表現: ${esc(v.avoid.join(' / '))} -->
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .info th{width:32%;color:var(--muted);font-weight:700}
  ...
  .note{margin-top:18px;font-size:12px;color:rgba(255,255,255,.78)}
...
<header id="hdr">
  <div class="wrap nav">
    <div class="brand">${esc(c.name)}<small class="en">${esc(v.label)}</small></div>
    <nav class="nav-links">
      <a href="#reasons">選ばれる理由</a>
      <a href="#services">サービス</a>
      <a href="#works">${esc(v.workLabel)}</a>
      <a href="#access">店舗案内</a>
    </nav>
    <a href="#cta" class="btn btn-primary">${esc(v.ctaLabel)}</a>
  </div>
</header>
...
<section class="hero">
  ...
  <div class="wrap">
    <div>
      ...
      <h1>${esc(v.headline(c))}</h1>
      <p class="lead">${esc(v.lead(c))}</p>
      <div class="btns">
        <a href="#cta" class="btn btn-primary">${esc(v.ctaLabel)}</a>
      ...
      <div class="chips">
        ...
        <span class="chip">🏷 ${esc(v.label)}</span>
        ${reviewChip}
      </div>
    </div>
...
<section class="sec" id="reasons">
  <div class="wrap">
    ...
    <div class="cards">${reasonCards(v)}</div>
  </div>
</section>
...
<section class="sec" id="works">
  <div class="wrap">
    ...
    <h2>${esc(v.workLabel)}</h2>
    <p class="lead-c">実際の写真に差し替えてご利用ください（著作権に配慮し、サンプルはプレースホルダーです）。</p>
    <div class="tiles">${workTiles(v)}</div>
  </div>
</section>
...
<section class="sec svcs">
  <div class="wrap">
    ...
    <h2>お客様の声</h2>
    <div class="voices">${voiceCards(v)}</div>
  </div>
</section>
...
<section class="sec cta" id="cta">
  <div class="wrap">
    ...
    <form class="form" onsubmit="return false">
      ...
      <button class="btn btn-light" type="submit">${esc(v.ctaLabel)}</button>
    </form>
...
<footer>
  <div class="wrap">
    <div><b>${esc(c.name)}</b><br>${esc(v.label)}｜${esc(c.area)}</div>
