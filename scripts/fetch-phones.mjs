...
if (existsSync(OUT)) {
  ...
  for (let r = 1; r < prev.length; r++) {
    ...
    // 確定結果(OK/番号なし/見つからない)だけ「済み」。DENIED/NETERR等は再実行で取り直す。
    if (pid && /^(OK|NO_PHONE|NOT_FOUND)/.test(st)) {
      done.set(pid, prev[r]);
    }
  }
}
...
const FIELD_MASK = 'id,displayName,nationalPhoneNumber,internationalPhoneNumber';
