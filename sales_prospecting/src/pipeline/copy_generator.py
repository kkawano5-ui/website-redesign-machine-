...
def generate_copy(config_path: str | None = None) -> dict:
    """
    Google Sheetsの「収集済み」行にAI生成の文面を書き込む。
    ...
    for row_num, row in enumerate(rows[1:], start=2):
        if cell(row, "送信ステータス") != TARGET_STATUS:
            continue

        company_info = {
            "company_name":       cell(row, "会社名"),
            "official_name":      cell(row, "現在の正式社名"),
            ...
            "latest_sns_date":    cell(row, "最新投稿日"),
            "email":              cell(row, "公開メールアドレス"),
            "contact_url":        cell(row, "問い合わせフォームURL"),
            "form_type":          cell(row, "フォーム種別"),
            "contact_method":     cell(row, "推奨連絡手段"),
        ...
        # フォーム専用企業のステータス分岐
        contact_method = cell(row, "推奨連絡手段")
        new_status = "フォーム送信待ち" if contact_method == "Form" else "文面生成済み"

        write_map = {
            "SNS上の改善余地":              copy.sns_improvement,
