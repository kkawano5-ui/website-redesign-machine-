...
text(s,L,Inches(1.95),Inches(7.1),Inches(2.6),
     [P(R("高城株式会社は神奈川県平塚市を拠点に、建築塗装・防水・改修工事を手がける会社（許可：神奈川県知事 第85139号、代表 小早川和廉様、「NO PAINT, NO LIFE.」）。",12.5,SOFT)),
      P(R("株式会社デザインワークス（設計・デザイン）と連携し、塗装・改修から設計まで一気通貫で対応するグループ、と理解しています。",12.5,SOFT)),
      P(R("従業員110〜120名・AI活用も着手済み——",12.5,SOFT),R("“使える組織”の土台はすでにある",12.5,SLATE,True),R("。次の一手は、過去の施工・設計・顧客情報という会社の資産をAIで活かすことです。",12.5,SLATE,True))],sa=8,ls=1.4)
for i,(v,u,lb) in enumerate([("110-120","名","グループ従業員"),("塗装×設計","","2社グループ"),("活用中","","公開APIも導入済み"),("施工×個人情報","","次の主戦場（④）")]):
    col=i%2; row=i//2
...
cols=[("📂","いま","紙・PDFの図面、見積書、施工写真、顧客台帳に、施工・設計情報と個人情報が“バラバラ・属人的”に眠っている",CHIP,SLATE),
      ("🤖","AIで移行","AI-OCR＋情報抽出で、図面・施工のメタ情報と顧客の個人情報を読み取り、構造化データに変換",SKY,BLUE),
      ("🗄️","これから","現場・設計・顧客が紐づいた検索可能なDBへ。「あの現場の前回施工は？」に即答できる資産に",RGBColor(0xE7,0xF0,0xE9),GREEN)]
...
pains=[("🔍","探せない・即答できない","過去の現場・図面・顧客履歴が探せず、問い合わせや再提案に時間がかかる"),
       ("🔁","塗り替え再提案の機会損失","塗装・防水は約10年周期の塗り替え需要。施工履歴がDB化されていれば“次の塗り替え”を先回り提案できる"),
       ("👤","属人化・退職リスク","“あの人しか知らない”状態。ベテラン退職で情報ごと失われる"),
       ("📑","紙・劣化リスク","図面・見積・施工写真の紙/個別保存は劣化・紛失・災害に弱い"),
       ("⚖️","個人情報の管理不安","個人宅オーナーの情報がどこに散在するか把握しきれず、漏えい・コンプラのリスク"),
       ("⏱️","対応スピード","塗り替え・メンテ・改修相談での即応力が、そのまま受注力になる")]
...
steps=[("①","取り込む","紙図面・PDF・見積書・施工写真・顧客台帳をスキャン／集約",CHIP,SLATE),
       ("②","AIで読む","AI-OCR＋抽出で、図面・施工メタ情報と個人情報をテキスト化・項目化",SKY,BLUE),
       ("③","個人情報を守る","氏名・住所等を自動検出し、マスキング／アクセス権限を付与",RGBColor(0xF3,0xE7,0xE6),RED),
       ("④","DBへ紐付け","現場 × 設計 × 顧客 を関連づけ、検索可能なデータベースへ",RGBColor(0xE7,0xF0,0xE9),GREEN)]
bw=Inches(2.78)
...
text(s,Inches(L.inches+0.3),Inches(5.56),Inches(11.3),Inches(0.8),
     [P(R("出口イメージ：",12.5,SLATE,True),R("「あの現場の前回塗装は？」「この顧客の施工履歴は？」に、社員の誰もが数秒で答えられる状態。",12.5,INK))],sa=0,ls=1.3)
...
secs=[("🔒","学習されないAIを選定","入力データをモデル学習に使わない商用APIを採用。情報が外部学習に流れない契約形態で運用"),
      ...
      ("📜","法令順守","個人情報保護法・建設業法等の要件に沿った取得・利用・保管・廃棄のルール整備"),
...
more=[("🔁","塗り替え再提案リストの自動生成","施工履歴から“そろそろ塗り替え時期”の顧客を抽出し、案内文を自動作成"),
      ("📋","見積書・施工計画書ドラフト","ひな型＋現場情報から下書きを自動生成、確認は人が行う"),
      ("📐","図面・現調からの情報抽出","図面や現地調査メモから数量・仕様を読み取り、積算・台帳へ自動入力"),
      ("💬","顧客問い合わせ一次対応","過去施工・FAQをもとに、問い合わせへ即時ドラフト回答"),
      ("🖼️","施工事例・提案資料の自動生成","施工写真とビフォーアフターから事例・提案資料を自動作成"),
      ("📊","社内ナレッジ横断検索","設計×顧客×現場を横断して“聞けば返ってくる”状態に")]
cw=Inches(3.78); ch=Inches(1.62)
...
qs=[("対象","まず何の書類・どの案件群から始めるか？（一番困っている / 一番数が多い）"),
    ("現状","図面・見積・顧客情報は今どんな形式か？（紙・PDF・既存システム・Excel）"),
    ("個人情報","社内の取扱いルール・NGライン・保管ポリシーは？クラウド利用の可否は？"),
    ("ゴール","“移行”の先に何をしたいか？（検索・塗り替え再提案・営業活用…優先順位）"),
