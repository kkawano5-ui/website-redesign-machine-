# みどりメモリアル霊園 向けアウトリーチ下書き

- 既存サイト: https://example-memorial.jp
- デモURL: https://demos.example.com/memorial-sample/
- 送信前に `gtm/outreach-templates.md` のコンプライアンスチェックを必ず確認

---

## 1. 初回（メール / 問い合わせフォーム）

件名: 【みどりメモリアル霊園様】Webサイトのリニューアル案を作成しました（無料・確認用）

ご担当者様

突然のご連絡失礼いたします。{自社名}の{氏名}と申します。
地方の中小企業様向けにWebサイトのリニューアル制作を行っております。

貴社サイトを拝見し、費用とプラン差が分かりにくく、比較検討で離脱しやすいという点が改善できると感じ、確認用のデモページを試作しました。
地方都市近郊で永代供養墓・一般墓・樹木葬を提供という強みが初見で伝わる構成を意識しています。

▼ デモ（みどりメモリアル霊園様向け・提案用）
https://demos.example.com/memorial-sample/

※既存の文章・画像は使用せず独自に再構成したサンプルです。費用は一切かかりません。
不要であればご放念ください。以後の案内が不要な場合はその旨ご返信ください。

ご興味があれば15分ほどで改善ポイントと費用感をご説明します。
ご都合のよい候補日を2〜3いただけますでしょうか。

------------------------------
{自社名} {氏名} / {電話} / {メール} / {住所}
------------------------------

## 2. フォローアップ（3〜5営業日後・1回のみ）

件名: Re: 【みどりメモリアル霊園様】リニューアルデモの件

ご担当者様

先日お送りしたデモページの件、いかがでしたでしょうか。
https://demos.example.com/memorial-sample/

「今は不要」「別担当へ」等でも構いませんので一言いただけますと幸いです。
（以後の案内が不要な場合はその旨ご返信ください。）
